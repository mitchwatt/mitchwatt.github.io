 ...
 **
 ** Version: 2.3.3 (local palette/marker extensions; see LOCAL-CHANGES.txt)
 **
...
*/
	...
	function configure( config ) {
		...
		if ( config.chalkEffect !== undefined ) chalkEffect = config.chalkEffect;
  ...
  function changeCursor( element, tool ) {
	...
	function createPalette( colors, length ) {
		...
		for ( var i = 0; i < length; i++ ) {
			...
			swatch.className = 'fa ' + ( colors[ i ].icon || 'fa-square' );
			swatch.setAttribute( 'aria-hidden', 'true' );
			colorButton.appendChild( swatch );
			colorButton.style.color = colors[ i ].paletteColor || colors[ i ].color;
			if ( colors[ i ].label ) {
				colorButton.title = colors[ i ].label;
				colorButton.setAttribute( 'aria-label', colors[ i ].label );
			}
 ...
 ******************************************************************/
	...
	// New drawings use the same slide-coordinate transform as loaded drawings.
	// Otherwise a first-session viewport-sized stroke shifts when reloaded.
	for ( var id = 0; id < drawingCanvas.length; id++ ) {
...
//console.log("Add drawing", newPDFPage);
	...
	function drawWithBoardmarker( context, fromX, fromY, toX, toY, colorIdx ) {
		...
		var marker = boardmarkers[ colorIdx ];
		context.save();
		context.lineWidth = marker.width || boardmarkerWidth;
		context.lineCap = marker.lineCap || 'round';
